...
public abstract class PhoneApp
{
    ...
    {
        float h = RowH;
        GUI.Label(new Rect(x, y, w, h), label, PhoneStyles.Heading);
        PixelGUI.Rule(x, y + h, w, new Color(PixelGUI.Gold.r, PixelGUI.Gold.g, PixelGUI.Gold.b, 0.5f));
        return h + PixelGUI.Px(3f);
    }

    // One text row on the phone: the phone's own face plus its leading, not the kit's full-size row.
    public static float RowH => PhoneStyles.LineH;
    ...
    {
        float h = RowH;
        var style = dim ? PhoneStyles.DataDim : PhoneStyles.Data;
